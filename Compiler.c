...
static void assign()
{
	...
	// get offset of variable on left side of assignment op
	int offset = (token-'a')*4;	next_token();
	// skip over assignment operator
	next_token();
	// evaluate expression on right side
	right_reg = expr();
	
	// write ILOC command to store value
...
{
	/* YOUR CODE GOES HERE */
	// can either be variable or exclamation mark
	if (is_identifier(token)) {	// signifies assignment
		assign();
	}
	else if (token == '!') {	// signifies print statement
		next_token();
		print();
	}
	else {	// bogus input
...
static void morestmts()
{
	...
	case '.':	// signifies end of program
		return;
		
	case ';':	// signifies end of statement
		next_token();
		stmtlist();
		return;
